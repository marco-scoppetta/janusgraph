...
public class FoundationDBTransaction extends AbstractStoreTransaction {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(FoundationDBTransaction.class);
    ...
    public FoundationDBTransaction(Database db, Transaction t, BaseTransactionConfig config, IsolationLevel isolationLevel) {
        super(config);
        tx = t;
        this.db = db;
    ...
    public synchronized void rollback() throws BackendException {
        ...
        if (LOG.isTraceEnabled())
            LOG.trace("{} rolled back", this.toString(), new FoundationDBTransaction.TransactionClose(this.toString()));
        try {
            tx.cancel();
            tx.close();
            tx = null;
    ...
    public synchronized void commit() throws BackendException {
        ...
        for (int i = 0; i < maxRuns; i++) {
            ...
            if (tx == null) return;
            if (LOG.isTraceEnabled())
                LOG.trace("{} committed", this.toString(), new FoundationDBTransaction.TransactionClose(this.toString()));
